...
public interface TaskPicker {
    ...
    /**
     * Have to pick runs from specified ones. Any picked run's engine requirements must much to specified
     * descriptor. Also each run's weight must be less or equal engine free capacity. Runs must have an order
     * in which engine have to start them. Engine will start iterate over returned runs and will start any
